function [prdY,Yr]  = crossvalK_drugATC( Adj, Sd, S_ATC, fold )

% the row of Adj represents drug, the column of Adj represents ATC code;
% Sd is drug similarity matrix; S_ATC is ATC code similarity
...
for k = 1 : fold
    ...
    for i=1:length(LabGrn)
        for j=1:length(LabDrn)
            Krn(i,j)=Sd(LabGrn(i),LabGrn(j))*S_ATC(LabDrn(i),LabDrn(j));
        end
    end
    ...
    for i=1:length(LabGst)
        for j=1:length(LabDrn)
            Kst(i,j)=Sd(LabGst(i),LabGrn(j))*S_ATC(LabDst(i),LabDrn(j));
        end
    end
%     [Hst] = Normal_M(Kst);
    Dst=[Y3 Kst];   formatKmatrix(Dst,'Dtest.txt');
      
  !C:\libsvm-3.32\windows\svm-train -s 0 -b 1 -t 4 -c 1 Dtrain.txt 
  % make sure software svm-train is under this address
      
  !C:\libsvm-3.32\windows\svm-predict -b 1 Dtest.txt Dtrain.txt.model result.out
  % make sure software svm-predict is under this address
